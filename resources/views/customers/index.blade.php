<x-app-layout>
    <div class="mx-auto max-w-7xl space-y-4 px-4 md:px-6 md:py-4 lg:px-8" x-data="customerManager()">
        {{-- HEADER --}}
        <div
            ...
            <a href="{{ route('customers.create') }}" class="hidden md:block">
                ...
                <div class="card">
                    <div class="card-header flex items-center justify-between">
                        <div class="flex gap-1 items-center">
                            ...
                            <p class="font-semibold">{{ $customer->code }}</p>
                        </div>
                    </div>
                    ...
                    <div class="card-body space-y-2">
                        ...
                        @if ($customer->address)
                            <p class="capitalize">{{ $customer->address }}</p>
                        @else
                        ...
                        <div class="flex justify-between items-center">
                                ...
                                @if ($customer->phone)
                                    @if (strpos($customer->phone, ',') !== false)
                                        <div class="flex flex-col">
                                            @foreach (explode(',', $customer->phone) as $phone)
                                                <p>{{ trim($phone) }}</p>
                                            @endforeach
                                        </div>
                                    @else
                            ...
                            <div class="flex gap-2">
                                <x-danger-button @click="initDelete({{ json_encode($customer) }})" class="btn-icon">
                                    <span class="w-6 h-6">
        ...
        {{-- DELETE MODAL --}}
        <x-modal name="delete-customer-modal" maxWidth="md">
            <div class="flex flex-col p-6 gap-2 md:gap-4">
                ...
                <form :action="actionurl" method="POST" class="flex items-center justify-end gap-2 md:gap-4">
                    @csrf
                    @method('DELETE')
                    <x-secondary-button @click="$dispatch('close-modal', 'delete-customer-modal')" class="btn">
                        Simpan Data
                    </x-secondary-button>
    ...
    <div
        ...
        <a href="{{ route('customers.create') }}" class="md:hidden">
